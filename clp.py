...
# Variáveis Globais
data_lock = threading.Lock() # bloqueio para dados compartilhados
new_target_event = threading.Event() # sinal de novo target disponível
current_drone_position = {'dx': 0.0, 'dy': 0.0, 'dz': 0.0}
new_target_received = {'tx': 0.0, 'ty': 0.0, 'tz': 0.0}
running = threading.Event() # parada global
running.set()

# Conexão ao OPC UA
# Configura os nós OPC UA
def setup_opc_nodes(url=OPC_UA_SERVER_URL):
    ...
    # Encontra as variáveis (drone x,y,z e target x,y,z)
    name_to_node = {}
    ...
    if not all(nodes_map.values()):
        ...
        raise RuntimeError(f"Váriaveis OPC não encontradas. Encontradas: {found}")
    print("[OPC Thread] Váriaveis OPC encontradas.")
    return client, nodes_map

# Thread 1: Cliente OPC
# le a posição do drone e escreve o novo target no OPC
def opc_ua_thread():
    ...
    try:
        ...
        while running.is_set():
            ...
            try:
                ...
                with data_lock: # Atualiza posição global
                    current_drone_position['dx'] = dx
                    current_drone_position['dy'] = dy
                    current_drone_position['dz'] = dz
            except Exception as e:
                print(f"[OPC Thread] Erro ao ler dados: {e}")
                running.clear() # Se der erro, desliga tudo
                
            # Escrita (Python -> OPC)
            if new_target_event.is_set(): # Verifica sinal
                new_target_event.clear() # Desliga sinal
                with data_lock:
                    local_target_copy = new_target_received.copy()
                try:
                    tx, ty, tz = local_target_copy['tx'], local_target_copy['ty'], local_target_copy['tz']
                    print(f"[OPC Thread] Escrevendo novo target: tx={tx}, ty={ty}, tz={tz}")
                    ...
                    print(f"[OPC Thread] Erro ao escrever target: {e}")
        ...
        print(f"[OPC Thread] Erro na inicialização: {e}")
    finally:
        if opc_client: opc_client.disconnect()
        print("[OPC Thread] Desconectado e parado.")
        running.clear() # Para as outras threads

# Thread 2: Servidor TCP/IP (Tarefas)
# Envia posição do drone para a interface gráfica
def tcp_sender_task(conn, addr):
    ...
    try:
        while running.is_set():
            ...
            # Formato: "timestamp,x,y,z\n"
            message = f"{timestamp_str},{dx:.4f},{dy:.4f},{dz:.4f}\n" 
            conn.sendall(message.encode('utf-8'))
            time.sleep(0.1) # Envia 10x por segundo
            
    except (BrokenPipeError, ConnectionResetError, OSError):
        print(f"[TCP Sender] Cliente {addr} desconectado.")
    finally:
        print(f"[TCP Sender] Task para {addr} parada.")

# Recebe posição do drone da interface gráfica
    ...
    try:
        ...
        while running.is_set():
            try:
                ...
                while '\n' in buffer: # Processa mensagens
                    message, buffer = buffer.split('\n', 1)
                    if not message: continue

                    try:
                        # Mensagem esperada: "x,y,z"
                        parts = message.split(',')
                        tx, ty, tz = float(parts[0]), float(parts[1]), float(parts[2]) 
                        print(f"[TCP Receiver] target recebido de {addr}: tx={tx}, ty={ty}, tz={tz}")
                        with data_lock: # Atualiza o target global
                            new_target_received['tx'] = tx
                            new_target_received['ty'] = ty
                            new_target_received['tz'] = tz
                        new_target_event.set() # Ativa o sinal e avisa a thread OPC
...
# Thread 2: Servidor TCP/IP (Principal)
def tcp_server_thread():
    ...
    try:
        ...
        while running.is_set():
            ...
            conn = None
            try:
                conn, addr = server_socket.accept() # Fica esperando um cliente
                print(f"[TCP Server] Cliente {addr} conectado. Servidor ocupado.")
                
                # Inicia as duas sub-threads (enviar e receber)
    ...
    try:
        while running.is_set():
            ...
            if not opc_thread.is_alive() or not tcp_thread.is_alive():
                ...
                running.clear() # para ambas as threads
                
    except KeyboardInterrupt:
        print("\n[Main] Desligamento solicitado (Ctrl+C).")
        running.clear() # Força parada das threads
